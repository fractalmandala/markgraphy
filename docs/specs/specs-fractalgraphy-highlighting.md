...
- Limited langs: `svelte, typescript, bash, css, json, markdown` (recommendation: only load what
  the site needs).
- Custom theme `fractalgraphy` built on the site's CSS variables so code re-themes with the
...
- Prerendered output: 47/47 `[slug]` pages contain `class="shiki fractalgraphy"` blocks with
