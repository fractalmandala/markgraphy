...
| DiagramEditor.svelte | `DiagramEditor.svelte` | 5 presets, glyph palette (6 groups), auto-align, live preview, copy as text / Svelte / `.svx` (imports rewritten to `'fractalgraphy'`); `bind:this` replaces `getElementById` | implemented — pending user evaluation |
